...
public final class DomainIndependentAgent implements SketchRecognitionAgent,
	...
	lineRecognizer = new LinePatternRecognizer(
		LineRecognitionParameters.DEFAULT_MIN_CORRELATION,
		LineRecognitionParameters.DEFAULT_MAX_AREA_RATIO);
	circleRecognizer = new CirclePatternRecognizer(
		CircleRecognitionParameters.DEFAULT_MIN_CORRELATION,
    ...
    private void recognize(final Stroke stroke,
	...
	if (!recognized) {
	    ...
	    LOGGER.info("Entering recursive recognition phase for stroke with "
		    + strokeSize + " points");
	    if (strokeSize > 1) {
